...
class performService:
    def __init__(self, servicer, inc, ecc, n):
        ...
        self.a = utility().getafromn(self.servicer['n'])
        self.vGEO = math.sqrt( utility().mu / self.a )
    ...
    def computeRevs(self, theta, n):
        ...
        T2 = ( 1 / n ) * ( theta + n * ( 2 * math.pi ) ) / utility().omegaE # s
        ...
        a2 = ( ( T2 * math.sqrt(utility().mu) ) / ( 2 * math.pi ) )**(2/3) # m
        ...
        if rp < ( utility().Re + self.minAlt ):
            return [10e5, 0]

        # compute dv of maneuver
        h2 = math.sqrt( 2 * utility().mu ) * math.sqrt( ( self.a * rc) / ( self.a + rc ) ) # m^2/s
        vp2 = h2 / self.a # m/s
